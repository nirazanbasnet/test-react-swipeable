		...
		<SwipeableViews axis="y" resistance className="h-full">
			<div className="flex flex-col justify-between h-full bg-gray-400">
				<div className="flex-1">
					...
					<figure className="relative mb-[2.4375rem] h-[20.25rem] w-[45rem] l:w-full">
                    <Image
                      blurDataURL={`${blogDetail?.blog ? blogDetail?.blog?.featured_image : '/static/default.jpg'}`}
                      placeholder="blur"
                      src={`${blogDetail?.blog ? blogDetail?.blog?.featured_image : '/static/default.jpg'}`}
